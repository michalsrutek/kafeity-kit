...
public class API<ErrorResponseType: Error> where ErrorResponseType: Decodable {
    ...
    func execute(request: RequestConvertible, successCodes: Set<Int>? = nil, responseHeaders: (([AnyHashable : Any]) -> Void)? = nil) -> Completable {
    ...
    func execute<Response: Decodable>(request: RequestConvertible, successCodes: Set<Int>? = nil, responseHeaders: (([AnyHashable : Any]) -> Void)? = nil) -> Single<Response> {
    ...
    func download(request: RequestConvertible) -> Single<Data> {
        let urlRequest = try! request.asURLRequest()
